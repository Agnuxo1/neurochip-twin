...
def main() -> None:
    ...
    seq = generate_sequence(2026, 0.82, frames=36)
    ...
    for i, frame in enumerate(seq.frames):
        ...
        draw.text((14, 10), f"NeuroChip Twin | frame {i+1}/{len(seq.frames)} | synthetic dose-response proxy", fill=(245, 245, 245))
        canvas = Image.new("RGB", (960, 600), (245, 247, 250))
        canvas.paste(image, (20, 20))
        _, table = phenotype_features(type(seq)(seq.frames[: i + 1], seq.dose, seq.label, seq.seed))
        ...
        draw2.text((690, 255), "No clinical claim", fill=(160, 45, 40))
