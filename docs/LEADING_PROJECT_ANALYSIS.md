...
We also added a real-data front-end check on BBBC038v1. After calibrating on 12 images and freezing the parameters, 24 held-out images gave mean IoU 0.515, Dice 0.573, precision 0.843 and recall 0.571. This improves the validation story compared with a synthetic-only submission, while the modest recall and non-OoC domain are disclosed rather than hidden. The next score-critical experiment is authorized OoC or matched perturbation data with measured chip/experiment IDs.
