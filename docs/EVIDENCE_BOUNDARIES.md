...
| CANDY human brain-on-a-chip study | Published human neuron/astrocyte chip with MeHg exposure, live-cell microscopy movies, time-course and dose conditions; candidate only | Independent chip/differentiation/movie groups and paired frame-level labels are not verified | Potential narrowly scoped external challenge of compartment-aware temporal features if permissions and grouping permit | Any NeuroChip Twin result, broad/unseen-toxicant generalization, multi-study robustness, or clinical prediction; no supporting files or measurements have been evaluated |
| MARC/iS3CC human neural-chip activity study | Human iPSC-derived cerebral tissue in a two-chamber chip; Penicillin G perturbation with paired pre/post calcium activity in treated and untreated chambers; candidate only | Figure caption reports six samples across six experiments, while Methods report at least six tissues across three experimental rounds; sample IDs and supplement mapping remain unchecked | Potential separate audit of calcium-event dynamics, synchrony, and cross-chamber propagation after replicate discrepancy, rights, and provenance review | Cytotoxicity/dose-response prediction, validation of the current toxicity head, or broad neural OoC generalization; no supplementary files or measurements have been evaluated |
| Human dual neural-organoid drug-screen image candidate | Mendeley record lists CC BY 4.0 bright-field images supporting hPCO drug screening and teratogenicity assessment; candidate only | Six archive batches are listed, but their relation to independent differentiations/acquisitions and image-level targets is unverified | Potential separately scoped static neural-organoid morphology audit after preregistration and group/label review | Organ-on-chip validation, longitudinal response prediction, established per-image labels, or any NeuroChip Twin metric; no images or measurements have been evaluated |
...
The CANDY brain-on-a-chip, MARC/iS3CC neural-chip activity study, human dual neural-organoid drug-screen archive, and human midbrain-organoid source are due-diligence candidates only; see [`EXTERNAL_DATASET_DUE_DILIGENCE.md`](EXTERNAL_DATASET_DUE_DILIGENCE.md) for their separate reuse-rights, grouping, target, and preregistration limits. CANDY remains the closest toxicity-focused domain match; iS3CC is the strongest candidate found for a distinct human neural-chip functional-activity audit, not toxicity validation. Neither is a benchmark for NeuroChip Twin until file rights and experiment-level sample mapping pass review. The hPCO archive is organoid bright-field drug-screen material—not OoC or verified time-lapse data. A separate human sensory-neuron MPS paper gives an architectural precedent for distinct neurite-toxicity and cytotoxicity outputs plus an orthogonal NF-L assay, but it is peripheral-neuron literature, not a public benchmark or validation of this project.

## Non-negotiable interpretation rules

1. **Do not pool labels across unpaired experiments.** The CANDY movies, iS3CC calcium-activity candidate, hPCO candidate, midbrain-organoid candidate, OoC quality images, rat DRG firing assay, BBBC038 nuclei masks, synthetic response sequences, and GSE255606 omics samples do not share matched sample-level image/response outcomes. They cannot be treated as one multimodal training or validation table.
...
- [CANDY human brain-on-a-chip neurotoxicity study](https://doi.org/10.1002/admt.202400107); [publisher article and supplementary-file list](https://onlinelibrary.wiley.com/doi/10.1002/admt.202400107)
- [MARC/iS3CC human cerebral-tissue chip activity study](https://doi.org/10.1038/s42003-022-03203-4); [publisher full text and supplementary files](https://www.nature.com/articles/s42003-022-03203-4)
...
- [Lu et al. human dual neural-organoid study](https://doi.org/10.1016/j.devcel.2026.08.013); [CC BY 4.0 bright-field image data record](https://data.mendeley.com/datasets/bm3726x9kp/1)
